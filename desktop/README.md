...
播放器支持暂停、精确 seek、音量/静音、速度、内封音轨与字幕、外挂字幕、全屏、快捷键、前后集、EOF 自动下一集和连续切换。进入新的播放页后，普通窗口会按片源显示比例和分辨率自动调整一次，并受当前屏幕可用区域限制；HiDPI 屏幕会换算逻辑尺寸，避免桌面缩放把窗口错误放大；首次适配后以用户手动尺寸为准，退出时恢复进入播放前的窗口。控制层仅由鼠标操作唤起，快捷键不会遮挡画面；播放进度保存异常以可关闭的短时提醒显示。后端仍负责 watched、续播位置和 next Episode 业务判断。

## 人工验收

1. 首页加载继续观看与正在追，切换五种收藏标签和“只显示本地已匹配”。
2. 进入 Subject，分别播放 4:3、16:9 和超宽片源；普通窗口应按原始显示比例和分辨率调整，超出屏幕时等比缩小，最大化/全屏时不强制改窗。退出播放后应恢复进入前的窗口尺寸和位置，视频仍是窗口内真正的 QML Item，resize/fullscreen 不错位。
3. 检查 H.264/HEVC、24/30/60 fps、内封/外挂字幕、音轨、速度、前后集与 EOF。
4. 播放中退出，重新打开同一 Episode，应从保存位置继续；从头播放应从 0 开始。播放至 EOF 后，从任意普通播放入口再次打开应从头开始且不报错。
...
9. 下载页应持续显示进度，暂停、继续、失败重试和删除 qBittorrent 任务有效；关闭并重启程序后未完成任务继续同步。对已完成的单集点击“更换片源”，选择新候选后，旧文件必须保留到新片源成功入库，再自动删除旧文件和旧 qBittorrent 任务；新下载失败时旧文件仍可播放。
...
23. 设置 `HTTP_PROXY`/`HTTPS_PROXY` 后启动客户端：Bangumi 同步、已看写回、`lain.bgm.tv` 封面、Comicat 和 AcgnX 搜索均应成功；KissSub、本地 FastAPI 与 qBittorrent 不应进入该代理。临时停止代理后触发一次自动已看，恢复代理并再次上报进度或执行 `BangumiSync`，遗漏状态应被补写。
